...
class SwapManager(Logger):
    ...
    async def _claim_swap(self, swap: SwapData) -> None:
        ...
        for txin in txos.values():
            ...
            if remaining_time <= 0:
                if remaining_time <= -2016:
                    # if there was a funding tx at one point we can already fail the htlcs, but leave some safety buffer
                    # for complete removal, so we can still refund it if the funding tx happens to show up at one point
                    swap.funding_txid, swap._payment_pending = None, False
