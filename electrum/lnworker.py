...
from .lnutil import (
    ...
    PaymentSuccess, ChannelType, LocalConfig, Keypair, ZEROCONF_TIMEOUT, UnblindedRoutingInfo, BlindedRoutingInfo
)
from .lnonion import (
...
class PaymentInfo:
    ...
    def validate(self):
        ...
        if self.min_final_cltv_delta is None:
            # when sending blinded payments we don't really know the *final* cltv delta
...
class SentHtlcInfo(NamedTuple):
    ...
    payment_id: bytes
    # trampoline: random per-trampoline secret
    # unblinded source routing: bolt11 payment secret
    # blinded source routing: None (we don't have a payment secret)
    payment_secret_bucket: Optional[bytes]
    amount_msat: int
    bucket_msat: int
...
class LNWallet(Logger):
    ...
    def __init__(self, wallet: 'Abstract_Wallet', xprv, *, features: LnFeatures = None):
        ...
        # invoice requests awaiting bolt 12 invoice response. path_id: concurrent.futures.Future[BOLT12Invoice]
        self._pending_bolt12_invoice_requests = {}  # type: dict[bytes, asyncio.Future[BOLT12Invoice]]
    ...
    async def pay_to_node(
        ...
        paysession.logger.info(
            ...
            f"route_info={"r_tags:" + str(BOLT11Addr.format_bolt11_routing_info_as_human_readable(routing_info.r_tags)) \
                            if not routing_info.blinded else str(len(routing_info.paths)) + " blinded path(s)"}. "
    ...
    async def pay_to_route(
        ...
        blinded_path = paysession.routing_info.get_current_path() if paysession.routing_info.blinded else None
        htlc = peer.pay(
            route=shi.route,
            chan=chan,
            ...
            blinded_path=blinded_path if not trampoline_onion else None,
    ...
    def suggest_payment_splits(
        ...
        *,
        ...
        recipient_pubkeys = routing_info.get_pubkeys()
    ...
    async def create_routes_for_payment(
        ...
        for sc in split_configurations:
            ...
            routes = []
            try:
                pubkey = routing_info.node_pubkey if not routing_info.blinded else routing_info.get_current_path().path.first_node_id
                is_direct_path = all(node_id == pubkey for (chan_id, node_id) in sc.config.keys())
                    ...
                    for (chan_id, _), part_amounts_msat in sc.config.items():
                        for part_amount_msat in part_amounts_msat:
                            ...
                            if not is_route_within_budget(
                                    ...
                                    cltv_delta_for_dest=routing_info.min_final_cltv_delta):
                                self.logger.info(f"rejecting route (exceeds budget): {route=}. {budget=}")
                                raise FeeBudgetExceeded()
                            shi = SentHtlcInfo(
                                route=route,
                                payment_id=routing_info.id,
                                payment_secret_bucket=routing_info.payment_secret if not routing_info.blinded else None,
                                amount_msat=part_amount_msat,
                            ...
                            min_final_cltv_delta = routing_info.min_final_cltv_delta if not routing_info.blinded else 0
                            routes.append((shi, min_final_cltv_delta, fwd_trampoline_onion))
            except NoPathFound:
                continue
    ...
    def create_route_for_single_htlc(
        ...
        if route[-1].end_node != dest_node:
            route_target = "blinded path introduction point" if routing_info.blinded else "invoice pubkey"
    ...
    def htlc_fulfilled(self, chan: Channel, payment_hash: bytes, htlc_id: int):
        ...
        if shi and htlc_id in chan.onion_keys:
            ...
            payment_key = payment_hash + shi.payment_id
    ...
    def htlc_failed(
        ...
        if shi and htlc_id in chan.onion_keys:
            ...
            payment_okey = payment_hash + shi.payment_id
    ...
    def create_onion_for_route(
        ...
        if trampoline_onion:
            assert not blinded_path, "blinded path should be in trampoline onion, not passed to outer onion"
            assert payment_secret, "Need outer payment secret for onion including the trampoline onion"
        else:
            assert bool(payment_secret) ^ bool(blinded_path), "requires either blinded path or payment secret"
        ...
        if not blinded_path:
            ...
            assert blinded_path.path and blinded_path.payinfo, blinded_path
            hops_data, blinded_node_ids, amount_msat, cltv_abs = calc_hops_data_for_blinded_payment(
                route,
                amount_msat,
                final_cltv_abs=final_cltv_abs,
                total_msat=total_msat,
                inv_path=blinded_path.path,
                inv_blindedpay_info=blinded_path.payinfo,
            )
    ...
    async def request_bolt12_invoice(
        ...
        *,
        ...
        """
        Creates an invoice_request, sends it to payee, returns Future to await the Invoice response.
        """
    ...
    def on_bolt12_invoice(self, recipient_data: dict, payload: dict):
        ...
        if invoice_error_tlv := payload.get('invoice_error', {}).get('invoice_error'):
            ...
            if pending_invoice_request:
                pending_invoice_request.set_exception(Bolt12InvoiceError.from_tlv(invoice_error_tlv))
            return
        ...
        if pending_invoice_request:
            pending_invoice_request.set_result(invoice)
            return
    ...
    def _verify_bolt12_invoice_requested_by_us(self, invoice: BOLT12Invoice) -> bool:
        ...
        our_sig = signed_invreq.encode(signing_key=signing_key, as_bech32=False)
    ...
    def _verify_bolt_12_offer_created_by_us(self, offer: BOLT12Offer) -> bool:
        """Verify that given bolt12 offer was created by us."""
        if not offer.offer_metadata or len(offer.offer_metadata) < 32:
            return False
        # the last 32 bytes should be our mac
        offer_mac, remaining_metadata = offer.offer_metadata[-32:], offer.offer_metadata[:-32]
        offer_without_mac = dataclasses.replace(offer, offer_metadata=remaining_metadata)
        encoded_offer = offer_without_mac.encode(as_bech32=False)
        our_mac = hmac_oneshot(key=self.bolt12_secret_key, msg=b'offer' + encoded_offer, digest='sha-256')
        if not util.constant_time_compare(our_mac, offer_mac):
            return False
        return True
    ...
    def on_bolt12_invoice_request(self, recipient_data: dict, payload: dict):
        ...
        if is_response_to_offer:
            ...
            reply_path_or_node_id = invreq.invreq_paths or invreq.invreq_payer_id
        ...
        try:
            if is_response_to_offer:
                offer: BOLT12Offer = extract_shared_fields(invreq, BOLT12Offer)
                # verify the offer was created by us
                if not self._verify_bolt_12_offer_created_by_us(offer):
                ...
                if offer.offer_paths:
                    ...
                    if encrypted_recipient_data not in (p.path[-1].encrypted_recipient_data for p in offer.offer_paths):
                        self.logger.debug(f"ignoring invoice_request arriving not on offer path")
                        return
                else:
                    # MUST ignore any invoice_request if it arrived via a blinded path.
                    if payload.get('path_id', {}).get('data') is not None:
                        self.logger.debug(f"ignoring invoice_request arriving on blinded path for unblinded offer")
                        return
                ...
                # TODO: this could be made deterministic so we can proof later on that we signed given invoice?
        ...
        try:
            invoice = BOLT12Invoice(
                **invreq.__dict__,
                ...
                invoice_relative_expiry=path_id_payload.relative_expiry,
