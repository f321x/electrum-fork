...
class LNWallet(Logger):
    ...
    async def pay_to_node(
        ...
        paysession.logger.info(
            ...
            f"route_info={"r_tags:" + str(BOLT11Addr.format_bolt11_routing_info_as_human_readable(routing_info.r_tags)) \
                            if not isinstance(routing_info, BlindedRoutingInfo) else str(len(routing_info.paths)) + " blinded path(s)"}. "
                ...
                if self.uses_trampoline() and not is_direct_path:
                    ...
                    for trampoline_node_id, trampoline_parts in per_trampoline_channel_amounts.items():
                        ...
                        trampoline_route, trampoline_onion, per_trampoline_amount_with_fees, per_trampoline_cltv_delta = create_trampoline_route_and_onion(
                            ...
                            min_final_cltv_delta=routing_info.min_final_cltv_delta,
                    ...
                    for (chan_id, _), part_amounts_msat in sc.config.items():
                        for part_amount_msat in part_amounts_msat:
                            ...
                            if not is_route_within_budget(
                                ...
                                cltv_delta_for_dest=None if blinded_path else routing_info.min_final_cltv_delta,
                            ...
                            min_final_cltv_delta = None if blinded_path else routing_info.min_final_cltv_delta
    ...
    def create_onion_for_route(
        ...
        total_msat: int,
        payment_hash: bytes,
        min_final_cltv_delta: Optional[int],  # for blinded payments we don't know the final cltv delta
        ...
        if trampoline_onion:
            ...
            assert bool(min_final_cltv_delta) ^ bool(blinded_path_info)
        ...
        if not blinded_path_info:
            ...
            assert cltv_abs > local_height, (cltv_abs, local_height)
