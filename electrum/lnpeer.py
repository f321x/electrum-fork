...
class Peer(Logger, EventListener):
    ...
    def send_onion_message(self, *, path_key: bytes, onion_message_packet: bytes):
        ...
        If called with the same message again, while its predecesssor wasn't sent yet, the superflous attempt gets dropped.
        """
        def add():
    ...
    def _check_accepted_final_htlc(
        ...
        if htlc.path_key:  # payment over blinded path
            ...
            if sha256(path_id_payload.payment_preimage) != htlc.payment_hash:
                log_fail_reason(f"sender misused our blinded path for unrelated payment? {path_id_payload}")
                raise exc_incorrect_or_unknown_pd

            # we could even go as far and only persist the information once we have received all htlcs
    ...
    def _fail_htlc_set(
        ...
        for mpp_htlc in list(htlc_set.htlcs):
            ...
            processed_onion_packet = self._process_incoming_onion_packet(
                onion_packet,
                payment_hash=payment_hash,
                path_key=mpp_htlc.htlc.path_key,
                is_trampoline=False,
            )
            if raw_error:
