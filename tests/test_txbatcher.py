...
class TestTxBatcher(ElectrumTestCase):
    ...
    async def test_to_sweep_after_anchor_sweep_conditions(self):
        ...
        to_sweep_no_tx = wallet.txbatcher.tx_batches['lnwatcher']._to_sweep_after(tx=None)
        ...
        to_sweep_no_conf = wallet.txbatcher.tx_batches['lnwatcher']._to_sweep_after(tx=None)
        ...
        with mock.patch.object(wallet.adb, 'get_tx_fee', return_value=2000), \
            ...
            to_sweep_high_fee = wallet.txbatcher.tx_batches['lnwatcher']._to_sweep_after(tx=None)
        assert not to_sweep_high_fee

        # after the ctx is confirmed the anchor shouldn't be broadcast anymore
        ...
        assert anchor_txin.prevout in wallet.txbatcher.tx_batches['lnwatcher'].batch_inputs
        to_sweep_ctx_conf = wallet.txbatcher.tx_batches['lnwatcher']._to_sweep_after(tx=None)
        assert not to_sweep_ctx_conf
        assert not wallet.txbatcher.tx_batches['lnwatcher'].batch_inputs
